...
export const sendPasswordChangedEmail = async (email, newPassword) => {
  ...
  if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
    console.log('SMTP non configure - notification changement mot de passe :');
    console.log(`Email        : ${email}`);
  ...
  const html = wrapHTML(`
    <p style="font-size:18px;font-weight:600;margin-bottom:12px">Votre mot de passe a ete modifie</p>
    <p style="font-size:15px;color:#4a5568;line-height:1.6;margin-bottom:20px">
      L'administrateur de <strong>${APP_NAME}</strong> a modifie votre mot de passe.
    ...
    <div class="note">
      <strong>Important :</strong> Si vous n'etes pas a l'origine de cette modification,
      contactez immediatement votre administrateur. Pour des raisons de securite,
      changez votre mot de passe des votre prochaine connexion.
    </div>
  `);
  ...
  await transporter.sendMail({
    from: `"${APP_NAME}" <${process.env.SMTP_USER}>`,
    to: email,
    subject: `${APP_NAME} - Votre mot de passe a ete modifie`,
    html,
  });
